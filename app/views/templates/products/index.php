            ...
            <?php if ($category->products) : ?>
                <?php foreach ($category->products as $product) : ?>
                    ...
                    <div class=" w-11/12  lg:w-full drop-shadow bg-white rounded p-2">
                        ...
                        <img class="block w-56 h-56 ml-auto mr-auto object-contain" src="/upload/<?= $product->photo; ?>" alt="<?= $product->name; ?>">
                        <hr>
...
<div class="flex">
    <?php for ($index = 1; $index <= $pages; $index ++) : ?>
        ...
        <?php if (isset ($_GET["page"]) && $index == $_GET["page"]): ?>
                <a href="?page=<?= $index; ?>" class="text-sky-600 font-medium"><?= $index ?></a>
            <?php else: ?>
                <a href="?page=<?= $index; ?>" class="text-sky-300"><?= $index ?></a>
            <?php endif; ?>
        </div>
    <?php endfor; ?>
</div>
